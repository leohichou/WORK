...
#
# 1. Get Endpoint ID by User Name
...
function GetEndpointIds <#There could be more than one endpoint associated with the user so we will find all of them#> {
	...
	Try{
		...
		While ($Flag){
			...
			$response.items | %{
				if (($_.associatedPerson.name -eq $User) -or ($_.associatedPerson.viaLogin -eq $User)){
					$machines += $_.id
				}
			}
...
function StartEndpointScan {
    ...
    Try {
		...
		If($Output.Status -eq "requested") {
			Write-Output "Scan has been requested sucessfully for $User at `'$($Output.requestedAt)`'"
			Write-Output "Please check `'Sophos Central Dashbord`' for Endpoint status."
		} Else {
			Write-Output "Scan cannot be started for $User `n Status: `'$($Output.status)`'"
		...
		If($_.Exception.Message -eq "InvalidCredentials") {
			...
			Write-Output "Endpoint for user `"$User`" was not found."
			Throw "ExecutionFailure"
		} ElseIf($_.Exception.Message -match "The remote server returned an error: \(400\) Bad Request*") {
			Write-Output "Scan cannot be started for `"$User`"."
			Write-Error "Trapped: $_"
			Throw "ExecutionFailure"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUXUqswBFvcnoaeEhQJF6coCbB
# o9aggg8CMIIEyjCCA7KgAwIBAgIQVYges/w+aNZI0+oa53/ULjANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQU1eUFcXnI
# v6gLzVUMUT2Z8CAu8HYwDQYJKoZIhvcNAQEBBQAEggEAP2w/y7xpQ7vWzsP7AiQw
# eEbTy9hyXMvELrJOuXrX/6yUkkyz5ZnC1m6F5LgOhQ6j5Ra5j570xvYIrTi1HtAG
# rjF6d71li/PR1Nhv6zoLq9I6cbCvJINWy9HTkxk37xJ9TJv3ViURyqmBNZjz5msE
# T4QgR+mFPyjI3FtExsytYq8L07Lani7Nmeu/goshnTHc5P3/bGmkAFCnHZOKX6yr
# hLVpxPN6ES8KEBcu6PUMVYjL1eU9ZU0gZwX6xS5IKQCVukf7VPeaMKMULUkccWxg
# 6UtmXducPqedw6npuWy/tu0hYS7rIrNn4qpoPnWQu7dt+9d16iANWQTOQNqSZjSY
# xA==
