#Requires -Version 7
# This script is used to add local site in web control policy in Sophos central 
# The following steps are performed:
#
# 1. Add local site in Web Control policy
#
#==========================================#
...
#Function add local site in web control policy
function AddLocalSite{
    $Token = GenerateToken
    $Header = @{
    ...
    $AllowItemUrl = "$BaseUrl"+"/endpoint/v1/settings/web-control/local-sites"
    ...
    try
	{
		if($ProxyFlag -eq 0){
			...
			if($Output.id){
				Write-Output "Url $Url added in Web Control policy"
			}else{
				Write-Output "Error while adding Url $Url in Web Control policy"
	...
	catch
	{
		...
		elseif ($ExceptionMessage -eq "The remote server returned an error: (400) Bad Request."){
			Write-Output "Invalid Url/Tag"
		}
        elseif ($ExceptionMessage -eq "Response status code does not indicate success: 409 ()."){
			Write-Output "Provided URL $Url is already added in the WebControl Policy. Please check the Sophos Central Dashboard."
		}
        else{
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU8v3QQ2jw5zeh85RgV3K1eyE/
# BvOggg8CMIIEyjCCA7KgAwIBAgIQVYges/w+aNZI0+oa53/ULjANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUR6VrWCSo
# gj9nmwyDfc3NRz+MEPUwDQYJKoZIhvcNAQEBBQAEggEAPYvFBK0K3Rr5xzu/Vuwo
# 4Bkqx6Ldh9lLbEoKG8xksYVsiDBj8YbPSCqYriIKn4zI+Iyt21IWOWzvZy2DjqOa
# b4lbUBIq2sKk70/QJntRV2cUmUgcC/o7UA/OVjJJalSeyuV4cvDeLsMwMCMMA0Tl
# Pg3XizQIOnNGGj2aQTjDDfCdfmJSlIgcTZbZoHwtgeT6w9CxPTxIJM+1uOrWn4kd
# 3zKtAdMwk+8aVXPZDs+Mauxi0NNXX41tU70G/iDfX8EvO35e3kv0WF1swQeUN2yE
# vpreMBV/1pQIK3qdl5PWOmRB/zb/IaODDYsdgr2R4G5I7iSdr1Niv6X5ourrmcDO
# hg==
