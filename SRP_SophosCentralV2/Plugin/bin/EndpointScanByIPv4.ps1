# Requires -Version 7
#
# This script is used to start scan on an Endpoint by IP v4 address
#
# The following steps are performed:
#
# 1. Get Endpoint ID by IPv4
# 2. Start Scan on Endpoint Id
#
#==========================================#
...
function GetEndpointIds <#There could be more than one endpoint associated with the user so we will find all of them#> {
	...
	Try{
		...
		While ($Flag){
			...
			$response.items | %{
				if ($_.ipv4Addresses.Count -ne 0){
					if ($_.ipv4Addresses.Contains($IPv4)){
						$EndpointInfo.ID += $_.id
						$EndpointInfo.HostName += $_.hostname
						$EndpointInfo.HostName += ", "
					}
				}
			}
...
function StartEndpointScan {
    ...
    Try {
        ...
        if($ProxyFlag -eq 0)
		{
		    $Output = Invoke-RestMethod -Uri $ScanEndpointUri -Method Post -Body "{}" -Headers $Header -ContentType "application/json" -SkipCertificateCheck
        }
        elseif($ProxyFlag -eq 1)
        {
            $Output = Invoke-RestMethod -Uri $ScanEndpointUri -Method Post -Body "{}" -Headers $Header -ContentType "application/json" -SkipCertificateCheck -Proxy $ProxyUrl
        }
        else{
            $SecureProxyPasswd = $ProxyPasswd | ConvertTo-SecureString -AsPlainText -Force
            $cred = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $ProxyUname, $SecureProxyPasswd
            $Output = Invoke-RestMethod -Uri $ScanEndpointUri -Method Post -Body "{}" -Headers $Header -ContentType "application/json" -SkipCertificateCheck -Proxy $ProxyUrl -ProxyCredential $cred
        }
			
			If($Output.Status -ne "requested"){
				Write-Output "Scan cannot be started for $($EndpointInfo.HostName) `n Status: `'$($Output.status)`'"
				Throw "ExecutionFailure"
			}
		}

		If($Output.Status -eq "requested") {
			Write-Output "Scan has been requested sucessfully for $($EndpointInfo.HostName) at `'$($Output.requestedAt)`'"
		...
		If($_.Exception.Message -eq "InvalidCredentials") {
			...
			Write-Output "Endpoint for IP $IPV4 was not found."
			Throw "ExecutionFailure"
		} ElseIf($_.Exception.Message -match "The remote server returned an error: \(400\) Bad Request*") {
			Write-Output "Scan cannot be started for `"$($EndpointInfo.HostName)`"."
			Write-Error "Trapped: $_"
			Throw "ExecutionFailure"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUEqA5CuBfPPFmvZHIvXlBIpZb
# cDuggg8CMIIEyjCCA7KgAwIBAgIQVYges/w+aNZI0+oa53/ULjANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUuGdjuvxS
# 6VQ/AkS6MVc0206Q4DUwDQYJKoZIhvcNAQEBBQAEggEAmdB+RHVpwvqtCTY+ucDl
# bvJrAhM6KIIJ07NlKrnbNOJxiKuRDkYGuhxYZpuXTKW5UCP1P7LStRxhfK7HFNuW
# nI0Gx43cYe75H+spqEnpqlYVqVcstSvx5NgpcBvd1ros5uZujSBTdJda9LFq4Fop
# vHU1CttOXgKqBVEmmfP+OTFgwge2HJoEllEvywvRKI3Qpmgboi0H2SLIcabQPtE0
# TqXnnrib8UzsNdTDM6OvWO9jEf2j41opFcOfqTLrcJiO/SUCtw6kygotrxHEQfvi
# 1U6WSSKMGzYxqt7NstRtguN9TYG7Y/XKEzEpMLe1z0s+I5d6f6UqDBrIoz6Fqkni
# zw==
