#Requires -Version 7
# This script is used to block files by sha256 in Sophos central 
# The following steps are performed:
#
# 1.Block files by sha256 
#
#==========================================#
...
#Function to block item in Sophos central
function BlockItem{
    $Token = GenerateToken    
    $Header = @{
    ...
    $Body = @{
      ...
      "comment" = "Blocked by Logrhythm SRP"
    }
    
    $Body = $Body | ConvertTo-Json

    $BlockItemUrl = "$BaseUrl"+"/endpoint/v1/settings/blocked-items"

    try
	{
		if($ProxyFlag -eq 0){
			$Output = Invoke-RestMethod -Uri $BlockItemUrl -Method Post -Headers $Header -Body $Body -ContentType "application/json" -SkipCertificateCheck
			if($Output.id){
				Write-Output "SHA256 $SHA256 added in blocked items list in Sophos Central"
			}else{
				Write-Output "Error while blocking SHA256 $SHA256"
				throw "ExecutionFailure"
			}
		}
		elseif($ProxyFlag -eq 1){
			$Output = Invoke-RestMethod -Uri $BlockItemUrl -Method Post -Headers $Header -Body $Body -ContentType "application/json" -SkipCertificateCheck -Proxy $ProxyUrl
			if($Output.id){
				Write-Output "sha256 $SHA256 added in blocked items list in Sophos Central"
			}else{
				Write-Output "Error while blocking SHA256 $SHA256"
			...
			$Output = Invoke-RestMethod -Uri $BlockItemUrl -Method Post -Headers $Header -Body $Body -ContentType "application/json" -SkipCertificateCheck -Proxy $ProxyUrl -ProxyCredential $cred
			if($Output.id){
				Write-Output "sha256 $SHA256 added in blocked items list in Sophos Central"
			}else{
				Write-Output "Error while blocking sha256 $SHA256"
	...
	catch
	{
		...
		elseif ($ExceptionMessage -eq "Response status code does not indicate success: 400 ()."){
            Write-Output "sha256 : $SHA256 cannot be added in blocked items list. Reason could be either of the following:"
            Write-Output "1. Invalid sha256`n2. Item is added in allowed items list"
			write-error $ExceptionMessage
			
		}
        elseif ($ExceptionMessage -eq "Response status code does not indicate success: 409 ()."){
			Write-Output "SHA256 $SHA256 already exists in blocked items list. Please Check the Sophos Central dashboard."
			write-error $ExceptionMessage
			
		}
        else{
            Write-Output "API Call error"
			write-error $ExceptionMessage
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUsY1PKIRHx9E8l0pTEjq7xSB6
# QVKggg8CMIIEyjCCA7KgAwIBAgIQVYges/w+aNZI0+oa53/ULjANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQURtFL49Mc
# O53cZGOlilKGCfOfh/owDQYJKoZIhvcNAQEBBQAEggEAlYLwik2O2iXETuLq3Cbv
# lqkW2QrX19N7FA/9zwzdcxGCMJrjQ9NXYbmjHH2YdnePaEs6Wppe+0yiNqjUyVAH
# THsTBOct+S8hy/R/dvL0Xh7JPT2AATLFPWmALB9VlsV1bcSJ04QLN/RzmEgcSEWc
# Kq86dwBoDhuHGxQu8qXSSOwbWXZILXmsFstXzm8UuL3nw36h/l9HOE1va/PDMxQ3
# n5L4/DWoCNAmRDHCcJ/HfzQejh7nqmU7Qr60yN4OE9hxFLjGBQX7xM1o328QOiG3
# POlcEnnU7ear+llD1nmVVF9b+o4F70X571Eu+9AlPWrhes54Cg5YFPSkLA3jDTdE
# zg==
