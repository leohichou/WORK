...
function GetEndpointId {
    ...
    Try {
        ...
        While($Flag) {
            ...
            If ($TotalItems -ne 0) {
                # Search for Endpoint matching hostname
                ...
                write-output "Provided hostname $HostName is not found. Please provide a valid hostname."
                exit 0
...
function StartEndpointScan {
        ...
        $ScanEndpointUri = $BaseUrl + "/endpoint/v1/endpoints/" + $EndpointId + "/scans"
	Try{
		if($ProxyFlag -eq 0){
        ...
        If($Output.status -eq "requested") {
            Write-Output "Scan has been requested sucessfully on Endpoint `'$HostName`' at `'$($Output.requestedAt)`'"
            Write-Output "Please check `'Sophos Central Dashbord`' for Endpoint status."
        } Else {
            Write-Output "Scan cannot be started on Endpoint `"$HostName`"."
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU1H/dOMVQEnywR/Snl6mH7y9I
# Fhiggg8CMIIEyjCCA7KgAwIBAgIQVYges/w+aNZI0+oa53/ULjANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUf1W5pjYF
# GJh75QeTU0mhUzHASt8wDQYJKoZIhvcNAQEBBQAEggEAQCLYSUsDz9LfxSNHRFWu
# EiwkAUZZywvd+2uv0g2rNyFVbRNa156wUxxMGuGDGFoLWGzL9BCc4dGomEbLYhiG
# VBxLoRwDELRpL0KclAaDl2P4G4rmY1hnfzxp82DRClnrYR7frAeh7kO4H/h17w9m
# ttrKLQlhLRd+fIeuM5Mk0J3xVztk9SaVKF/mdEyfHYraceh9KmYOWD83dNHr+mwF
# ei4IH0Kod24k/Th+D9DzcBZzmkCPqi2FeH0xeauGPohbygo+gNuM+k0Oi3UWHN7/
# Jx7hWskX6vXI0S5sxIa4CE/GekpGBbufEAkMawakxD+UCswinldg8IkR5mNlv97E
# nw==
