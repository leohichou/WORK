...
# This action kills a process on a given Windows host using Job Manager Service credentials.
#
# The following steps are performed:
#
# 1.Process is killed on given host machine
#
#==========================================#
...
# If local, or no credentials passed, build appropriate CIM object
...
if($localHost -like $TargetHost -OR $localIP -like $TargetHost) {
    $process = Get-CimInstance -Class Win32_Process -Filter "$filter $operator '$TargetProcess$wildcard'" | Invoke-CimMethod -MethodName Terminate
}else{
    $process = Get-CimInstance -Class Win32_Process -Filter "$filter $operator '$TargetProcess$wildcard'" -ComputerName $TargetHost | Invoke-CimMethod -MethodName Terminate
}
if(!($process)){
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUN6ZMCx7KhuvZBpSQuL25cbCC
# Lnmggg8CMIIEyjCCA7KgAwIBAgIQVYges/w+aNZI0+oa53/ULjANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUsubmeJso
# N8mC174WQwEItHibZEgwDQYJKoZIhvcNAQEBBQAEggEATB2vv1R2XzbATGlPFqxw
# rns9yunAzVJi4jAFNHPiJM0io+/7V/wryvtZD05ShRETiFxou+hu9MjYGNuhDOPt
# 6uHzTF4EvVdCa5I5rT5JtQT06RnNKx3tC7ujTlW1NM1ESK2gL1imh8a6EjD5o7LY
# j2+Ul3IBHfS+mRgtmLezhIKByWIpfYf46mp5x9KnFLusMW4UoAZVA4KiPBkXN7sU
# qON+wr6yZLDLtBBaOhjv+z8WTyzeofSxKRq4MWqRCVybMEmNTctBZNPqlJR8V+3X
# UZmZ4fpPXkZrnBBI8Q69hPGcwuj4RDwQRyoaP+DQhr6GwmQunGQYGsyPp+eU0kFN
# Pw==
