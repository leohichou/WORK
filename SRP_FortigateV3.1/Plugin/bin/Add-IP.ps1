...
#
# Add IP to Logrhythm Blocklist Group
#==========================================#
...
#*******************************************
#Function to Check if IP already Added or Add IP to fortigate
Function Add-IP 
{
 $Members = Get-GroupInfo -Group $Group -VDOM $VDOM
 If($Global:IPFlag -eq 1){
  ForEach($Member in $Members){
   If($Member.name -eq $Global:IPName){
    Write-Host "`n$Object Already Added in Group - $Group.`n"
    Exit
   }
  }
  #Add IP to Group
  Add-ToGroup $Group $Global:IPName
 }
 Else{
  $Name = SubAdd-IP
  #Add IP to Group
...
#Function to Add IP to Address only
Function SubAdd-IP
{
  ...
  If($Global:IPRange -eq 1){
  $Body = "{ `"vdom`" : `"$VDOM`" , `"json`" : { `"name`" : `"$Name`", `"subnet`" : `"$Object`" } }"
  }
  Else{
  $Body = "{ `"vdom`" : `"$VDOM`" , `"json`" : { `"name`" : `"$Name`", `"subnet`" : `"$Object/32`" } }"
  }
  ...
  Else
  {
   Throw "HTTP Error. Failed to add $Object to Address."
  }
}
#*******************************************
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU5mfRdosate2/2ofQsRpQ4T9X
# VH+gghebMIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# AQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBRNwf45tNV8mE2kqyBQ
# Vs/x8keU6TANBgkqhkiG9w0BAQEFAASCAQA/WDqUbtwV7L405UNOwg7KaN4V/1Tb
# ZWqUpgSYSuO/v7CUVR4GHmj10N18pNffEM5vRm74uo7zOf/dm7LfG7Dp1mU4ln2f
# uji44xS3lo0R7pSMOx5InckNVRS85tryTjjpzLm+d9E2rvs2R4ZW2IJCgTBuX7hp
# egKv4rOdR0Qf7xIvAihKTsKUujwhOEa3aTe0rF+wvqtB0QnOYDu/VuKBQ319MUHr
# LqFkgBegt483Fm4yXY1HD5S48luLKQokEKvaJhKqRwzgKqtPUaM+KXoaZiO/cuQE
# Sxd4lDmNm4A8vgELJX7JeXY32C2//ObnBSGYeoEXr4jK6Ya48jL/lc/GoYICCzCC
...
# MQ8XDTIwMTIwMzA5NTgxMlowIwYJKoZIhvcNAQkEMRYEFKarQwFg/o5zIV6D9kCv
# vhq9erKAMA0GCSqGSIb3DQEBAQUABIIBAAJfoDt1Py0CHTkAvr/mUJwokePy4mrg
# Rnn0GHGPgtlyTLHf9ShVPoYGHyeVge+PkhJJxt938Alg/a05XfUSMm0HykbsnRip
# QpGdSk4fN+jGwkQoGNigwdq/Pdlw2rchCPIScJSLc59gn8eZQ9W57aYFv9PMrIvf
# zK+8yHRQ2cRdox5PDOLWQ+9I/2+rSTJS5KOm+VcF11FfdekMdx149V8D/9XO7zNB
# OoosyXLWppTnQhAl0NRcuccYsQbur7sTcuwLwcQbjU9DKmWx2rfJ77nC1PpSlPcJ
# Nuwbuvg23AwpLU95i2AA3DO0McGUcdjkUoNmdorCtwqXYyQ9nQ3Eufc=
