...
#
# 1. Checks if AD Module exists and if it doesn't exists Install and Import it.
# 2. Validate Target Account Name, Admin login User Name.
# 3. Validate and Get Domain 
...
# Function to Check SmartResponse Config File
...
#Function to Validate Admin Login User Name
function ValidateAdminLoginUserName{
	if($AdminLoginUserName -Notmatch $AdminLoginUserNameRegex) {
		...
		$Creds = New-Object System.Management.Automation.PSCredential (($AdminLoginUserName), $SecurePassword)
	}
    return $Creds
}
...
function GetDomain {
	$Creds = ValidateAdminLoginUserName
    try {
		$ADDomainController = Get-ADDomainController -Credential $Creds
		
	}
	catch{
		Write-Output "Invalid Credentials or limited privileges. Please check Admin Username and password again."
...
function NewHashtable
{
	$HashTable = [PSCustomObject]@{
								"AdminLoginUserName" = $SecureAdminLoginUserName
...
{
	NewHashtable | Export-Clixml -Path $ConfigurationFilePath
	Write-Output "Validations Passed."
	Write-Output "Configuration Parameters saved for AD Account Management V3."
}



$AdminLoginUserName = $AdminLoginUserName.trim()
$AdminPassword = $AdminPassword.trim()
$AdminLoginUserNameRegex = "^[^\/\\\]:;|=,+*?<>]{1,30}$"

$ConfigurationDirectoryPath = "C:\Program Files\LogRhythm\SmartResponse Plugins"
$ConfigurationFilePath = "C:\Program Files\LogRhythm\SmartResponse Plugins\ADAccountManagementV3ConfigFile.xml"
...
$SecureAdminLoginUserName = $AdminLoginUserName | ConvertTo-SecureString -AsPlainText -Force
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUcyvRK84bhPq87Di3OH8aAEif
# 1qCggg8CMIIEyjCCA7KgAwIBAgIQVYges/w+aNZI0+oa53/ULjANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUe4q8nK84
# wvIRbfWhqmr5BYHTI3MwDQYJKoZIhvcNAQEBBQAEggEAeFaoRMY/WcCAz4pLthgz
# phGe49rL8aPE1D8HHREDBggjoYVt52fYug8G8ZDhF4DQEVh5ZuExH8zHAZ6zgjew
# Dq2T8gGEUzBu7nID3ok2MNgn6is3LGK19R3Tx8noLgrSaqi94OzeKJ/HMR9X2AAm
# EvZIzZS2+16m07v8Xqc1NbYRVMCyb8pP1oi2uAG5YzyNeVdEdScJGLULWdM1CaWm
# INkVwKgKpROvT7UC9yc9MgljRhEc7J95AOikE5yQmsTHEMlywZspuiZ6I8txglye
# SR/WUzNJ8g7ltbhWq/mxEEsU5oQhIzTwmbe61mNFwX7eX+4UPD57+R4JUEsaV3aR
# hA==
