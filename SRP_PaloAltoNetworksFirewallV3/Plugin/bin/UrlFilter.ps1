...
# This script is used to filter URL by given category
#
# The following steps are performed:
#
# 1. Validate the required inputs
# 2. Makr a request to add the URL Filter
...
#Fuction to add URL Filter rule
Function AddUrlFilter {
    $AddFiterUri = "$PANFirewallHost"+"/restapi/$PANVersion/Objects/URLFilteringSecurityProfiles?location=vsys&name=$($FilterName)&vsys=$Vsys"
    $Member= @{
            "member"= @($URLBlockArr)
        }
    ...
    Try {
        if($ProxyFlag -eq 0)
		{
		    $UriReturn = Invoke-RestMethod -Uri $AddFiterUri -Method Post -Headers $Header -Body $Body -SkipCertificateCheck
        }
        elseif($ProxyFlag -eq 1)
        {
            $UriReturn = Invoke-RestMethod -Uri $AddFiterUri -Method Post -Headers $Header -Body $Body -SkipCertificateCheck -Proxy $ProxyUrl
        }
        else{
            ...
            $UriReturn = Invoke-RestMethod -Uri $AddFiterUri -Method Post -Headers $Header -Body $Body -SkipCertificateCheck -Proxy $ProxyUrl -ProxyCredential $cred
        }
        if($UriReturn.msg -eq "command succeeded"){
            Write-Output "URL Filter $FilterName added successfully"
        }
    }
    catch {
        $ErrorMessage = $_.Exception.Message
        if($ErrorMessage -eq "Response status code does not indicate success: 409 (Conflict)."){
            Write-Output "Rule name $FilterName already exists. Please provde a new URL Filter name to continue."
	    throw "ExecutionFailure"
        }
        elseif($ErrorMessage -eq "Response status code does not indicate success: 400 (Bad Request)."){
            Write-Output "Please check the input value(s) for BlockList Category and try again."
	    throw "ExecutionFailure"
        }
        else{
...
$URLBlockList = $URLBlockList.Replace(" ","")
$URLBlockArr = $URLBlockList.split(",")
$FilterName = $FilterName.Trim()
if($Description -ne "" -and $null -ne $Description){
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUhhqwB35JG5jz7Wp1XbS081J2
# gZyggg8CMIIEyjCCA7KgAwIBAgIQVYges/w+aNZI0+oa53/ULjANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQU6/cGijRc
# erTeK0LZs/NWMhHtFLwwDQYJKoZIhvcNAQEBBQAEggEAhiXtGeVIqAsb7Al2O1Zb
# P2XLL5/F5fnBetiv+5IkAvAgpNQMwSXWzJjbHSdgjuzhHNk/VQ3ms/SvU/+W03MP
# LK0KUlWiEJVWJxBV3Zl238TWuANhLPVN3kD37g6eokhGNMb89jQ9omkF9wHNvUUA
# 47Jd0r25prsxdrv/SrSQzTVPJv8ltGqQbIlHkL4qD8ynlX6FifwKjWuqSpHn122s
# i1vIcAEsn6oHgvrhcLaomKseLfCTb44+YHvZfKl+b0nsrp5kF7e9eYk2D6qEBhSp
# jcY33GEwgscoBEeLIQMwWsrj1+bbAnQGREyVtHfG565XYmddoV4W91YyEOxgW4nG
# Gw==
