...
# This script is used to add IP to group
#
...
#Fuction to create IP Object
Function CreateIPObject {
    $IPObjectUrl = "$PANFirewallHost"+"/restapi/$PANVersion/Objects/Addresses?location=vsys&vsys=$Vsys&name=$IP"
    $Body = @{
        "entry"= @{
            "ip-netmask"= $IP
            "@name"=  $IP
    ...
    Try {
        if($ProxyFlag -eq 0)
		{
		    $IPObject = Invoke-RestMethod -Uri $IPObjectUrl -Method Post -Headers $Header -Body $Body -SkipCertificateCheck
        }
        elseif($ProxyFlag -eq 1)
        {
            $IPObject = Invoke-RestMethod -Uri $IPObjectUrl -Method Post -Headers $Header -Body $Body -SkipCertificateCheck -Proxy $ProxyUrl
        }
        else{
            $SecureProxyPasswd = $ProxyPasswd | ConvertTo-SecureString -AsPlainText -Force
            $cred = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $ProxyUname, $SecureProxyPasswd
            $IPObject = Invoke-RestMethod -Uri $IPObjectUrl -Method Post -Headers $Header -Body $Body -SkipCertificateCheck -Proxy $ProxyUrl -ProxyCredential $cred
        }
        ...
        if($ErrorMessage -eq "Response status code does not indicate success: 400 (Bad Request)."){
            Write-Output "Invalid IP $IP entered"
        }else{
            Write-Output $ErrorMessage
        }
        exit 1
...
#Fuction to check if IP Object Exists
Function CheckIfIPObjectExists {
    ...
    foreach($IPValue in $IPs){
        if($IPValue.$IPNetMask -eq $IP){
            $IPName = $IPValue.$Name
            $flag = 1
            break
        }
...
#Fuction to add IP to group
Function AddIPToGroup {
    $IPObjectAddUrl = "$PANFirewallHost"+"/restapi/$PANVersion/Objects/AddressGroups?location=vsys&vsys=$Vsys&name=$GroupName"
    $IPArray = @()
    $IPArray+= $AlreadyAddedMembers
    $IPArray+= $IPName
    $Member= @{
            "member"= @($IPArray)
        }
    ...
    Try {
        if($ProxyFlag -eq 0)
		{
		    $IPObject = Invoke-RestMethod -Uri $IPObjectAddUrl -Method Put -Headers $Header -Body $Body -SkipCertificateCheck
        }
        elseif($ProxyFlag -eq 1)
        {
            $IPObject = Invoke-RestMethod -Uri $IPObjectAddUrl -Method Put -Headers $Header -Body $Body -SkipCertificateCheck -Proxy $ProxyUrl
        }
        else{
            $SecureProxyPasswd = $ProxyPasswd | ConvertTo-SecureString -AsPlainText -Force
            $cred = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $ProxyUname, $SecureProxyPasswd
            $IPObject = Invoke-RestMethod -Uri $IPObjectAddUrl -Method Put -Headers $Header -Body $Body -SkipCertificateCheck -Proxy $ProxyUrl -ProxyCredential $cred
        }

        Write-Output "IP $IP successfully added to Group $GroupName"
    }
    catch {
        $ErrorMessage = $_.Exception.Message
        if($ErrorMessage -eq "Response status code does not indicate success: 409 (Conflict)."){
            Write-Output "IP $IP is already added in Group $GroupName"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU6qQhKxsD2QTVZ93YCgtNR54I
# 9pCggg8CMIIEyjCCA7KgAwIBAgIQVYges/w+aNZI0+oa53/ULjANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUJvDCahcN
# UhL2MRAvE2MlbBClMxowDQYJKoZIhvcNAQEBBQAEggEAle4q/pQL4sVcabSx1ul6
# qVMlyT30UgxqF3Kejwi9goj0LzY2gs8a1f+CA411UCKggs864Q4gXcigMYLB5qgm
# laLvRSdeup90Nuv0hKtYc5BL6N+pYfZulLUAUodvqDPkV1hzEb5/drHCUoKrTumP
# xk5y7uJaK6rohiHpJP3SSs853lTd9zLEgPWtoDjHT6ccvVqbyap4m/9DyfcZ/LNe
# p8BU8lbSxdWlc8N3sGezineMlJYrIiocc/y3MHljKI3RBkRoczVuLCnLnD8h2MSb
# sohstY8lGLlEiWMQl5vTnMhOXLqjV1XNt94FZj35Z8BLIJ2MxAM74XMvrYjM/Htj
# rw==
