...
# This action kills a process on a given Windows host using user-supplied credentials.
#
# The following steps are performed:
#
# 1.CIM session is created
# 2.Process is killed on given host machine
#
#==========================================#
...
if($TargetProcess -match '^0x\w+$') {
$HexProcess = $TargetProcess
...
# If remote and alternative credentials passed, build modified CIM object with credentials
$SecurePassword = new-Object System.Security.SecureString
$AdminPassword.ToCharArray() | % { $SecurePassword.AppendChar($_) }
$Credential = New-Object -typename System.Management.Automation.PSCredential -argumentlist "$AdminUserName",$securePassword
$Session = New-CimSession -ComputerName $TargetHost -Credential $Credential -SkipTestConnection
$process = Get-CimInstance -Class Win32_Process -Filter "$filter $operator '$TargetProcess$wildcard'" -CimSession $Session | Invoke-CimMethod -MethodName Terminate


if(!($process)){
    Write-Output "Invalid Process : $TargetProcess"
    throw "ExecutionFailure"
}else{
    Write-Output "Process $TargetProcess killed on $TargetHost"
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU68G2wC4wAMzvObNKop1/1fVs
# yZ+ggg8CMIIEyjCCA7KgAwIBAgIQVYges/w+aNZI0+oa53/ULjANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUxdy8ORFO
# xrUxlhPMs6RLXj2z/t0wDQYJKoZIhvcNAQEBBQAEggEApbxrQYq9Jm6opx8dYhG4
# E9lUfhCZk3hm39qzSRjhU6a6G9FETXwl3dtYIs6ak6NWAstvPX+yFuK1vGbO/nTy
# CwQb+WbFlcKwzDgPqOU66SntX/B/paSSbl/Jfont5TNU/Va+67eDYVEbVbrfV4td
# 6s04vjBqNv4Uy3lNVNXSmd/2lQ3I1PneutGEPAx4vzYRJczxDw07TsqDKjun6fVr
# IF5urZqSeaTEErYDmFD5fJuazUqNiMjcT6/8btLl52VMqhbhk7xkiN6CMXQkQjRf
# ASUeVUaa+U15qHuBZ6cI/8mUvwrL03CyeyA7TXwL+NYYgw6dEsY3o6R/rmHZlfmX
# Yw==
